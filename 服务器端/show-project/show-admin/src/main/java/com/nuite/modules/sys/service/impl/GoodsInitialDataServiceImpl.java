...
public class GoodsInitialDataServiceImpl extends ServiceImpl<GoodsInitialDataDao, GoodsInitialDataEntity> implements GoodsInitialDataService {
    ...
    /**
     * 区域某一品类的已经采购量（首单+已经补单）
     */
	@Override
	public Integer getFirstOrderAndPatchedNum(Integer areaSeq, Integer periodSeq, String sxValue) {
		Wrapper<GoodsInitialDataEntity> wrapper = new EntityWrapper<GoodsInitialDataEntity>();
		wrapper.setSqlSelect("SUM(FirstOrderNum) + SUM(PatchedNum)");
		wrapper.where("BranchOfficeSeq = {0} AND PeriodSeq = {1} AND SXValue = {2} ", areaSeq, periodSeq, sxValue);
		List<Object> list = goodsInitialDataDao.selectObjs(wrapper);
	...
	@Override
	public List<Map<String, Object>> getAllPeriods(Integer year) {
		return goodsInitialDataDao.getAllPeriods(year.toString());
	}
