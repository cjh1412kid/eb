          ...
          if (data.result.length > 1) {
            if (checkedNode(data.seq)) {
              ...
              for (var i = 0;i < tags.length;i++) {
                  if(tags[i].toHtml().startsWith("<div")) {
                      var child = tags[i].children;
                      for(var j = 0;j < child.length;j++) {
                          content += child[j].toHtml();
                      }
                  }else {
                      content += tags[i].toHtml();
                  }
              }
              ue.setContent(content);
              setTimeout(function () {
...
UE.Editor.prototype.getActionUrl = function (action) {
  if (action == 'uploadimage' || action == 'uploadscrawl' || action == 'uploadimage') {
    return '/show-admin/ueditor/imgUpload?&action=' + action;
  } else if (action == 'uploadvideo') {
    return '/show-admin/ueditor/videoUpload?&action=' + action;
  } else {
...
function setContent2(content) {
    var root = UE.htmlparser(decodeURIComponent(content), true);
    var tags = root.children;
    content = "";
    for (var i = 0;i < tags.length;i++) {
        if(tags[i].toHtml().startsWith("<div")) {
...
function getContentAndUpload() {
  ...
  var totalWidth = 0;
  for(var i = 0;i < tags.length;i++) {
    if(contents[i].startsWith("<div")) {
      newContent += contents[i];
      continue;
    }
    if(height === 0) {
      var divTitle = "<div style='height:";
    }
    var img = new Image();
    var imgWidth = parseInt(tags[i].getAttr("width"));
    img.src = tags[i].getAttr("src");
    var naturalHeight = img.height;
    var imgNext = new Image();
    var heightNext = 0;
    if(i < tags.length - 1) {
        imgNext.src = tags[i + 1].getAttr("src");
        heightNext = imgNext.height;
    }
    if(imgWidth == 100) {
        height += naturalHeight;
        imgContent += contents[i];
        if(height + heightNext > 1000) {
            newContent += divTitle + height + "px;'>" + imgContent +"</div>";
            height = 0;
            imgContent = "";
        }
    }else {
        totalWidth += imgWidth;
        contentTemp += contents[i];
        if(totalWidth == 100 || i === tags.length - 1) {
            height += naturalHeight;
            totalWidth = 0;
            imgContent += contentTemp;
            contentTemp = "";
        }else {
          continue;
        }
        if(height + heightNext > 1000 || i === tags.length - 1) {
            newContent += divTitle + height + "px;'>" + imgContent +"</div>";
            height = 0;
            imgContent = "";
        }
...
function table() {
  ...
  $("#jqGoodsGrid").jqGrid({
    ...
    colModel: [
      ...
      {name: 'shoeId', width: 120, align: "center"},
      {
...
vm = new Vue({
  ...
  methods: {
    ...
    loadCategory: function () {
      // 加载分类树
      $.get(baseURL + "sowingMap/category", function (r) {
...
function ok() {
  UE.dom.domUtils.setAttributes(focusNode2, {
    ...
    'tapmode': "active",
    'onclick': 'Bridge.postMessage(\'' + vm.sowingMap.type + ';' + vm.sowingMap.linkSeq + '\')',
...
function loadUeditorRecords(callback) {
  $.get(baseURL + 'ueditor/getUeditorRecords', function (r) {
