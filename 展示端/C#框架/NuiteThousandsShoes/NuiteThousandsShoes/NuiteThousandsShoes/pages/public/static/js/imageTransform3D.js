...
ge1doot.transform3D.Triangle = function (parent, p0, p1, p2) {
  this.ctx = parent.ctx;
  this.produceId = parent.produceId; //图片相关联产品的id
  this.hotTitle = parent.hotTitle; //图片相关联产品的id
  this.texture = parent.texture;  //parent -> Image
  this.p0 = p0;
  this.p1 = p1;
  this.p2 = p2;
...
ge1doot.transform3D.Triangle.prototype.draw = function () {
  if (this.p0.visible || this.p1.visible || this.p2.visible) {
    ...
    try {
      /*画图*/
      // this.ctx.drawImage(this.texture, 0, 53, 547, 330);
      this.ctx.drawImage(this.texture, 0, 0);

      // this.ctx.font = "50px Verdana";
      // var gradient = this.ctx.createLinearGradient(0, 0, 640, 0);
      // gradient.addColorStop("0", "magenta");
      // gradient.addColorStop("0.5", "blue");
      // gradient.addColorStop("1.0", "red");
      // // 用渐变填色
      // this.ctx.fillStyle = gradient;
      // this.ctx.fillText(this.hotTitle, 10,50);
    } catch (e) {

      this.ctx.fillStyle = "white";
      this.ctx.fillRect(0, 0, 640, 386);
      ...
      // 用渐变填色
      this.ctx.fillStyle = gradient;
      this.ctx.fillText(this.produceId, 50, 250);  //图片加载失败，显示图片相关产品编号
    }
...
ge1doot.transform3D.Image = function (parent, imgSrc, lev, callback) {
  this.parent = parent;
  this.produceId = this.parent.id;
  this.hotTitle = this.parent.hotTitle;
  this.points = [];
  this.triangles = [];
  ...
  /*设置图片元素固定的宽高*/
  this.texture.width = 640;
  this.texture.height = 386;
