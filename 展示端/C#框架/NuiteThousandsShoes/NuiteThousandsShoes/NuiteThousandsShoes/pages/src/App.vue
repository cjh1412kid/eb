<template>
    <div id="app" class="screen">
        ...
        <Festival :festival-animation="festivalAnimation" v-if="festivalAnimation"></Festival>
        <!--消息提示-->
        <div id="message" v-if="message" v-text="message"></div>
        <!--背景墙-->
        <div class="bgWall" :style="[screenStyle]" v-show="isShowBGWall"></div>
        <div class="videoBGWall" :style="[screenStyle2]" v-if="isLoadVideoBGWall"></div>
    </div>
</template>
...
<script>
  ...
  import Festival from './components/Festival2.vue'
  ...
  export default {
      ...
      findFestivalAnimateName2: function () {
        ...
        window.NUITE.getAnimate().then(function (result) {
          ...
          if (result && result.length > 0) {
            // console.log("获取节日动画数据: ", result);
            var jsonObj = JSON.parse(result);
            var jsonObj2 = JSON.parse(jsonObj);
            // console.log("jsonObj: ",jsonObj,typeof jsonObj)
            console.log("节日动画数据对象: ",jsonObj2,typeof jsonObj2)
            var res = vm.festivalAnimation = jsonObj2;

            //判断自定义背景图是否存在
            if (res && res.backgroundEntity && res.backgroundEntity.material) {
              //初始化视频背景
              //获得背景图地址
              var material = res.backgroundEntity.material;
    ...
    watch: {
      showType: function (val, oldVal) {
        ...
        //是否显示图片背景墙，图片轮播时
        this.isShowBGWall = val == 0 ? false : true;
...
<style>
    ...
    .bgWall {
        ...
        filter: opacity(.3);
        /*background: -webkit-linear-gradient(to bottom right, #eee, #cba);*/
        /*background: linear-gradient(to bottom right, #eee, #cba);*/
    }
