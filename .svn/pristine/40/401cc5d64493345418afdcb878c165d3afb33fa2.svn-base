...
public class EBlanErpTask {
    ...
    public void copyErpData() {
		...
		try {
			Date endTime = DateUtils.addDateDays(new Date(), -2);
			String endTimeStr = dateFormat2.format(endTime);
			logger.info("~~~~~~~~~~erp 复制数据，本次打算复制数据的截止时间gxrq为：" +  endTimeStr);
			erpService.copyDataFromErp(endTimeStr);
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("~~~~~~~~~~erp 复制数据出错:" + e.getMessage(), e);
		}
		
		long copyendTime = System.currentTimeMillis();
        logger.info("~~~~~~~~~~erp 复制数据结束 用时:" + (copyendTime - copystartTime));
    }

    @Scheduled(cron = "0 20 1 * * ?")   //注意，1点作业会同步区域，此时跑可能会出现因Area表被清库而报错的问题，所以改为1:20
    public void erpData() {
        if (lock.tryLock()) {
            lock.lock();
            try {
                ...
                logger.info("~~~~~~~~~~erp 同步鞋子，结束， 耗时:" + (endTime - startTime));
    ...
    public boolean shoeSync() {
        Date updateTime = (Date) shoeService.selectObj(new EntityWrapper<ShoeEntity>()
                ...
                .eq("Del", 0));
        logger.info("~~~~~~~~~~erp 同步鞋子，本地最大更新日期:" + dateFormat.format(updateTime));
    ...
    public boolean shopShoeStock() throws Exception {
        ...
        while(w2sData.size() == 0 && DateUtils.daysBetween(w2sMaxDate, new Date()) >= 5) {
        	//如果间隔4天以上还没有取到数据，认为那一天不会再有数据，开始取下一天的数据
        ...
        while(s2wData.size() == 0 && DateUtils.daysBetween(s2wMaxDate, new Date()) >= 5) {
        	//如果间隔4天以上还没有取到数据，认为那一天不会再有数据，开始取下一天的数据
        ...
        while(s2sData.size() == 0 && DateUtils.daysBetween(s2sMaxDate, new Date()) >= 5) {
        	//如果间隔4天以上还没有取到数据，认为那一天不会再有数据，开始取下一天的数据
	    ...
	    while(saleData.size() == 0 && DateUtils.daysBetween(saleMaxDate, new Date()) >= 5) {
	    	//如果间隔4天以上还没有取到数据，认为那一天不会再有数据，开始取下一天的数据
    ...
    private ShoesDataDailyDetailEntity getDetailFromErpEntity(ErpStockEntity entity, boolean from) {
        ...
        if (shoeEntity != null) { //当货号不存在时，进出库表直接填null，保留shoesId即可（货号不存在情况很多，可能15年前、可能不是鞋子）
        ...
        detailEntity.setUpdateTime(resolveOutDate(entity.getOutDate()));
        detailEntity.setAreaSeq(areaSeq);
        detailEntity.setBranchOfficeSeq(branchOfficeSeq);
        detailEntity.setInputTime(entity.getUpdateTime());
        return detailEntity;
    }
...
//        	return null;
        ...
        ShoeEntity shoeEntity = shoeService.selectByGoodId(shoesId);
        Integer shoeSeq = shoeEntity.getSeq();
        ...
        saleData.setSaleDate(resolveOutDate(entity.getOutDate()));
        saleData.setShoeSeq(shoeSeq);
        ...
        saleData.setInputTime(entity.getUpdateTime());
        saleData.setDel(0);
        saleData.setSaleType(0);
        saleData.setErpOrderNum(entity.getStockNum());
        saleData.setColor(colorEntity.getSeq());
        saleData.setSize(sizeEntity.getSeq());
        return saleData;
    }
    ...
    private ShoeEntity getLocalShoeDetail(ShoeDetail shoeDetail) {
        ...
        if (yearProperty != null && yearProperty.getPropertyName() != null) {
            ...
            logger.error("~~~~~~~~~~erp 同步鞋子，组装鞋子数据，鞋子年份:" + year);
        }
        if (year == null) {
            logger.error("~~~~~~~~~~erp 同步鞋子，组装鞋子数据，鞋子年份是null:" + shoeDetail.getShoeId() + ", 直接返回 null，取消同步此鞋子");
            return null;
        }
        ...
        if (seasonProperty != null && seasonProperty.getPropertyName() != null) {
            ...
            logger.error("~~~~~~~~~~erp 同步鞋子，组装鞋子数据，鞋子季节:" + season);
        }
        //获取波次信息
        ...
        if (season != null) {
            ...
            if (periodEntity == null) {
            	logger.info("~~~~~~~~~~erp 同步鞋子，组装鞋子数据，发现波次不存在，货号" + shoeDetail.getShoeId() + "，波次表插入一条新波次:" + year + season);
        	...
        	logger.info("~~~~~~~~~~erp 同步鞋子，组装鞋子数据，鞋子季节是null:" + shoeDetail.getShoeId() + ", 继续同步此鞋子，鞋子波次填null");
        }
    ...
    //将outdate字符串解析成date
    private Date resolveOutDate(String outDate) {
