...
var vm = new Vue({
	...
	data : {
		...
		sizeAllotNowShoeSizeNumsList:[], //当前正在打开配码页面配码的鞋子 配码预览 每个尺码的 总进货量
		sizeAllotNowShoeSizeStocksList:[], //当前正在打开配码页面配码的鞋子 配码预览 每个尺码的 库存
		sizeAllotNowShoeSizeSaleNumsList:[], //当前正在打开配码页面配码的鞋子 配码预览 每个尺码的 总销量
		sizeAllotNowShoeSizeAllotNumList:[null,null,null,null,null,null,null,null,null,null,null], //当前正在打开配码页面配码的鞋子 配码预览 每个尺码的已经配码量
		nbCode:1,				//配码页面模板南北选择
		...
		countryTotalPlanPurchaseNum:'',	//全国此品类总的计划采购量
		countryFirstOrderAndPatchedNum:'',	//全国已经采购量（此品类首单+已经补的订单）
		countryRestPlanPurchaseNum:'', 		//全国剩余采购量（本次配码不应该超过这个数量）
		branchofficeTotalPlanPurchaseNum:'',	//分公司此品类总的计划采购量
		branchofficeFirstOrderAndPatchedNum:'',	//分公司已经采购量（此品类首单+已经补的订单）
		branchofficeRestPlanPurchaseNum:'' 		//分公司剩余采购量（本次配码不应该超过这个数量）
	},
	methods : {
		getUser: function(){
		...
		periodSelect:function(period){
			if(vm.pageCheckedShoeIds > 0 || vm.pageCheckedSizeAllotNum > 0){
				layer.confirm("未提交的补单、配码数据将被清空，确认要切换条件吗？", {
		            btn: ["确定","取消"]
		        }, function(index){
		...
		getData:function(){
			...
			if(sxValue && startDate && endDate && periodSeq){
				...
				$.ajax({
					...
					success : function(r){
							if(r.code == 1){
								vm.countryTotalPlanPurchaseNum = r.result[0].countryTotalPlanPurchaseNum;
								vm.countryFirstOrderAndPatchedNum = r.result[0].countryFirstOrderAndPatchedNum;
								vm.countryRestPlanPurchaseNum = r.result[0].countryRestPlanPurchaseNum;
								...
								if(vm.user.roleSeq == 3){
									vm.branchofficeTotalPlanPurchaseNum = r.result[0].branchofficeTotalPlanPurchaseNum;
									vm.branchofficeFirstOrderAndPatchedNum = r.result[0].branchofficeFirstOrderAndPatchedNum;
									vm.branchofficeRestPlanPurchaseNum = r.result[0].branchofficeRestPlanPurchaseNum;
							    ...
							    vm.pageCheckedShoeIds = 0;
								vm.pageCheckedSizeAllotNum = 0;
							}
		...
		sizeAllotChangeCount:function(sizeTemplate) {
			...
			var templateDetailArr = sizeTemplate.templateDetailArr;  //配码详情
			
			//将临时修改的件数，放入临时修改配码详情map，待点击"确认"时存入页面配码详情
			if(vm.sizeAllotDetailInLayer.hasOwnProperty(seq)){ //如果已存在，修改原有的值，并计算已配码的变化量，修改sizeAllotAlreadyNum
				...
				var changeCount = count - oldCount;
				var changeNum = changeCount * perNum;
				...
				//计算每个尺码量，加到配码预览每个尺码已配量
				for(var i = 0; i < templateDetailArr.length; i++){
					if(templateDetailArr[i] > 0){
						if(vm.sizeAllotNowShoeSizeAllotNumList[i] == null){
							vm.sizeAllotNowShoeSizeAllotNumList[i] = templateDetailArr[i] * changeCount;
						} else {
							vm.sizeAllotNowShoeSizeAllotNumList[i] = vm.sizeAllotNowShoeSizeAllotNumList[i] + templateDetailArr[i] * changeCount;
						}
					}
				}
				...
				for(var i = 0; i < templateDetailArr.length; i++){
					if(templateDetailArr[i] > 0){
						if(vm.sizeAllotNowShoeSizeAllotNumList[i] == null){
							vm.sizeAllotNowShoeSizeAllotNumList[i] = templateDetailArr[i] * count;
						} else {
							vm.sizeAllotNowShoeSizeAllotNumList[i] = vm.sizeAllotNowShoeSizeAllotNumList[i] + templateDetailArr[i] * count;
						}
					}
				}
		...
		submitReplenish:function(){
			...
			var alertMessage = "已选择 <b>" + vm.pageCheckedShoeIds + "</b> 个货号，总配码量 <b>"+ vm.pageCheckedSizeAllotNum + "</b> 双<br/>";
			//判断本次配码量是否超过分公司剩余采购量
			if(vm.pageCheckedSizeAllotNum > vm.branchofficeRestPlanPurchaseNum){
				alertMessage += "<span style = 'color: red'>注意：本次配码量已超过分公司剩余计划采购量! </span><br/>仍然";
			}
			alertMessage += "确定要提交总部？";
...
function checkboxOnclick(checkbox){
	...
	if (checkbox.checked == true){
	    ...
	    getTemplates(); //获取配码模板，并加入可能已经配码的件数
	    getShoeSizeStocks();  //获取鞋子当前各个尺码的库存
	    calculateSizeAllotNowShoeSizeAllotNumList() //计算预览的各个尺码配码数量
...
//请求后台全部模板List，计算预览的尺码配码数量
function calculateSizeAllotNowShoeSizeAllotNumList() {
	var sizeAllotNowShoeSizeAllotNumList = [null,null,null,null,null,null,null,null,null,null,null];
	$.get(baseURL + "sys/sizeAllotTemplate/getTemplates", function(r) {
        if (r.code === 1) {
        	var resultList = r.result;
			...
			if(vm.sizeAllotDetailInLayer){
				...
				for (var key in sizeAllotDetailMap) {
					...
					for (var i = 0; i < resultList.length; i++) {
						...
						if(templateSeq == seq){
							//计算每个尺码量，加到配码预览每个尺码已配量
							var templateDetailArr = sizeTemplate["templateDetailArr"];
							for(var j = 0; j < templateDetailArr.length; j++){
								if(templateDetailArr[j] && templateDetailArr[j] > 0){
									if(sizeAllotNowShoeSizeAllotNumList[j] == null){
										sizeAllotNowShoeSizeAllotNumList[j] = templateDetailArr[j] * count;
									} else {
										sizeAllotNowShoeSizeAllotNumList[j] = sizeAllotNowShoeSizeAllotNumList[j] + templateDetailArr[j] * count;
									}
								}
							}
			...
			vm.sizeAllotNowShoeSizeAllotNumList = sizeAllotNowShoeSizeAllotNumList;
        } else {
        	vm.sizeAllotNowShoeSizeAllotNumList = sizeAllotNowShoeSizeAllotNumList;
        	alert(r.msg);
        }
...
//请求后台每个尺码的库存
function getShoeSizeStocks() {
	$.get(baseURL + "sys/shoesData/getShoeSizeNumStockSale?shoeSeq=" + vm.sizeAllotNowShoeSeq + "&areaType=" + vm.user.roleSeq + "&areaSeq=" + vm.user.userAreaSeq, function(r) {
		if (r.code === 1) {
			vm.sizeAllotNowShoeSizeNumsList = r.result[0].numArr;  //总进货量
			vm.sizeAllotNowShoeSizeStocksList = r.result[0].stockArr;  //库存
			vm.sizeAllotNowShoeSizeSaleNumsList = r.result[0].saleNumArr;  //总销量
		} else {
			alert(r.msg);
		}
	});
